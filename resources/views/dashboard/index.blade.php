...
@section('title', 'Dashboard - Gestion de Parc Automobile')
...
<div class="datatable-card">
    ...
    <script>
        ...
        // Remplir dynamiquement les navires et agences
        function fillNavires() {
            fetch('/ships')
                .then(res => res.json())
                .then(data => {
                    const select = document.getElementById('id_nav');
                    select.innerHTML = '<option value="">Sélectionner un navire</option>';
                    data.forEach(ship => {
                        select.innerHTML += `<option value="${ship.id}">${ship.name_nav}</option>`;
                    });
                });
        }
        function fillAgencies() {
            fetch('/agencies')
                .then(res => res.json())
                .then(data => {
                    const select = document.getElementById('id_agency');
                    select.innerHTML = '<option value="">Sélectionner une agence</option>';
                    data.forEach(agency => {
                        select.innerHTML += `<option value="${agency.id}">${agency.name_agency}</option>`;
                    });
                });
        }
        fillNavires();
        ...
        saveBtn.addEventListener('click', function() {
            ...
            if (form.checkValidity()) {
                ...
                const travelData = {
                    ...
                    id_nav: document.getElementById('id_nav').value,
                    id_agency: document.getElementById('id_agency').value
                };
                fetch('/travels', {
                    method: 'POST',
                    headers: {
